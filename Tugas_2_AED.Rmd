...
```{r}
ggplot(DA.NOTCA,aes(x=`Consumer prices ( avg annual avg. % growth 5yrs)`)) +
  facet_wrap(`sub-region` ~ ., nrow=4) + geom_density()
```
```{r}
DA.NOTCA
con_eas_as <- DA.NOTCA[DA.NOTCA=="Eastern Asia",
                       "Consumer prices ( avg annual avg. % growth 5yrs)"]
con_soe_as <- DA.NOTCA[DA.NOTCA=="South-eastern Asia",
                       "Consumer prices ( avg annual avg. % growth 5yrs)"]
con_south_as <- DA.NOTCA[DA.NOTCA=="Southern Asia",
                       "Consumer prices ( avg annual avg. % growth 5yrs)"]
con_wes_as <- DA.NOTCA[DA.NOTCA=="Western Asia",
                       "Consumer prices ( avg annual avg. % growth 5yrs)"]
